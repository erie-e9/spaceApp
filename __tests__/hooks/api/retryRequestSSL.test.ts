...
describe('sslFetch', () => {
    ...
    it('should retry and succeed within the retry limit', async () => {
        (retryRequest as jest.Mock).mockResolvedValueOnce({ success: true, result: { data: { success: true } } });

        await sslFetch('/test', {}, mockBaseQuery, mockEnqueueRequest, 3);

        expect(retryRequest).toHaveBeenCalledWith({ url: '/test', method: undefined, headers: undefined, body: undefined }, mockBaseQuery, 3);
    ...
    it('should enqueue request after retry limit is reached', async () => {
        ...
        expect(retryRequest).toHaveBeenCalledWith({ url: '/test', method: undefined, headers: undefined, body: undefined }, mockBaseQuery, 3);
        expect(mockEnqueueRequest).toHaveBeenCalledWith({ url: '/test', method: undefined, headers: undefined, body: undefined });
