...
describe('retryRequest', () => {
    ...
    it('retries the request up to the specified limit when a temporary error occurs', async () => {
        fetchMock.mockRejectOnce(new Error('Network Error')) // Primer intento falla
    ...
    it('logs errors on each failed attempt', async () => {
        ...
        expect(Logger.error).toHaveBeenCalledWith('[retryRequest] Request failed. Retrying...', expect.any(Object));
    });
