...
import { clearTasks, updateTask as updateTaskSlice } from '@slices/private';
...
export const useTasks = () => {
  ...
  const [patchTask, { isLoading: patchLoading }] = usePatchTaskMutation();
  ...
  const getTaskByIdHook = useCallback(
    (id: number | string) => {
      try {
        const task = tasks.find((taskItem: Task) => taskItem.id === id);
        return task;
      } catch (error) {
  ...
  const updateTaskHook = useCallback(
    async (task: Task) => {
      try {
  ...
  const patchTaskHook = useCallback(
    async (task: Task) => {
      try {
        await dispatch(updateTaskSlice(task));
  ...
  const deleteTaskHookAction = useCallback(
    async (id: Pick<Task, 'id'>, callback?: () => void) => {
      try {
  ...
  const deleteTaskHook = useCallback(
    async (id: Pick<Task, 'id'>, callback?: () => void) => {
      await confirmRemoveActionAlert(async () => {
        await deleteTaskHookAction(id, callback);
      });
    },
    [confirmRemoveActionAlert, deleteTaskHookAction],
  );
  ...
  return {
    data: tasks,
    getTaskHook: useGetTaskByIdQuery,
    addTaskHook,
    updateTaskHook,
    patchTaskHook,
    deleteTaskHook,
