        ...
        try {
          ...
          dispatch(
            apiTasks.util.updateQueryData('getTasks', task, (draft: any) => {
              const index = draft.findIndex((taskItem: Task) => taskItem.id === updatedTask.id);
              if (index !== -1) {
    ...
    patchTask: builder.mutation<Task, Partial<Task>>({
      ...
      async onQueryStarted(task, { dispatch, queryFulfilled }) {
        try {
          ...
          dispatch(
            apiTasks.util.updateQueryData('getTasks', task, (draft: any) => {
              const index = draft.findIndex((taskItem: Task) => taskItem.id === patchedTask.id);
              if (index !== -1) {
        ...
        try {
          ...
          dispatch(
            apiTasks.util.updateQueryData('getTasks', undefined, (draft: any) => {
              const index = draft.findIndex((taskItem: Task) => taskItem.id === id);
              if (index !== -1) {
