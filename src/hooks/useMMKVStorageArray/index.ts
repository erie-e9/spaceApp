...
export const useMMKVStorageArray = <T extends { id: string }>(options: StorageHookOptions<T>) => {
  ...
  const updateMMKVItem = useCallback(
    (id: number | null, updatedItem: Partial<T>) => {
      ...
      if (id === null || typeof id === 'undefined') {
        throw new Error('A valid idRequest or index must be provided to update an item.');
      }
    ...
    [getMMKVData, saveMMKVData, showQueueUpdatedToast],
  );
  ...
  const removeMMKVItem = useCallback(
    (id: number | null) => {
      ...
      if (id === null || typeof id === 'undefined') {
        throw new Error('A valid idRequest or index must be provided to remove an item.');
      }
