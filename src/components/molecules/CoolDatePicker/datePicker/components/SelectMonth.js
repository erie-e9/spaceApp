...
const SelectMonth = () => {
  ...
  const onChangeYear = (text) => {
    if (text === '') {
      ...
      const [maxYear] = maximumDate.split('/').map(Number);
      setYear(utils.toPersianNumber(maxYear));
  ...
  const isMarkedMonth = useCallback(
    (month) => {
      const dateValue = `${utils.toEnglish(year)}/${String(month + 1).padStart(2, '0')}`;
      return markedMonths && markedMonths.includes(dateValue);
    },
    [year],
  );

  const isBlockedMonth = useCallback(
    (month) => {
      const dateValue = `${utils.toEnglish(year)}/${String(month + 1).padStart(2, '0')}`;
    ...
    <Animated.View style={containerStyle}>
      ...
      <View style={[style.monthList, utils.flexDirection]}>
        {[...Array(12).keys()].map((item) => {
          const disabled =
            utils.checkSelectMonthDisabled(mainState.activeDate, item) || isBlockedMonth(item);
          const marked = isMarkedMonth(item);
          return (
            <TouchableOpacity
              key={item}
              activeOpacity={0.8}
            ...
            >
              ...
              {marked && (
                <View
                  style={[
                    style.markedDot,
                    currentMonth === item + 1 && style.markedDotSecondary,
...
const styles = (theme) =>
  StyleSheet.create({
    ...
    itemText: {
      ...
      fontSize: theme.textFontSize * 0.9,
